...
def finetune(model, retain_dataloader, forget_loader, device, num_epochs=1, lr=5e-5, seed=0):
    ...
    for epoch in range(num_epochs):
        ...
        for x_retain, y_retain in tqdm(retain_dataloader):
...
def cf_k(model, retain_dataloader, forget_dataloader, device, num_epochs=1, lr=5e-5, freezed_layers = 1, seed=0, unfreezed_encoder_layer = -1):
...
def neggrad(model, retain_dataloader, forget_dataloader, device, num_epochs=1, lr=5e-5, seed=0):
    ...
    for epoch in range(num_epochs):
        ...
        for x_forget, y_forget in tqdm(forget_dataloader):
...
def advancedneggrad(model, retain_dataloader, forget_dataloader, device, num_epochs=1, lr=5e-5, seed=0):
...
def unsir(model, retain_dataloader, forget_dataloader, device, batch_size, num_epochs_1 = 1, num_epochs_2 = 1, lr=5e-5, noise_lr = 5e-4, seed=0):
...
class SCRUBTraining:
    def __init__(self, teacher, student, retain_dataloader, forget_dataloader, T, device, lr, seed=0):
    ...
    def train_epoch(self):
        ...
        for x_retain, y_retain in self.retain_dataloader:
            ...
            # Loss computation
            loss_cls = self.criterion_cls(outputs_retain_student, y_retain)
        ...
        for x_forget, _ in self.forget_dataloader:
...
def scrub(model, retain_dataloader, forget_dataloader, device, num_epochs=1, T=4.0, lr=5e-5, seed=0):
    ...
    scrub_trainer = SCRUBTraining(teacher, model, retain_dataloader, forget_dataloader, T, device, lr, seed=seed)
...
def unlearning_step(model, unlearning_teacher, full_trained_teacher, unlearn_data_loader, optimizer, 
    ...
    losses = []
    for x, y in unlearn_data_loader:
...
def bad_teaching(model, unlearning_teacher, full_trained_teacher, retain_data, forget_data, device, epochs = 1,
                optimizer = 'adamW', lr = 5e-5, batch_size = 8, KL_temperature = 1, seed = 0):
...
def srl(model, retain_dataset, forget_dataset, bs, device, num_epochs=1, lr=5e-5, seed=0):
...
def saliency_map_generation(model, retain_dataloader, forget_dataloader, device, save_path, treshold = 0.5, num_epochs=1, lr=5e-5, seed=0):
    ...
    for x, y in tqdm(forget_dataloader):
